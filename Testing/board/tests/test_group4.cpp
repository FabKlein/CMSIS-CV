...
#define GRAY8_HISTOGR_UT(NB_BINS, NUM_BOUNDS_EDGES, BOUNDS_EDGES, UNIFORM, WITH_MASKS, WITH_ACCUM)                     \
    {                                                                                                                  \
        uint8_t bounds[NUM_BOUNDS_EDGES] = ARR(BOUNDS_EDGES);                                                          \
                                                                                                                       \
        status = test_gray8_histogr(inputs, wbuf, total_bytes, testid, cycles, NB_BINS, bounds, UNIFORM, WITH_MASKS,   \
                                    WITH_ACCUM);                                                                       \
    }


static arm_cv_status test_gray8_histogr(const unsigned char *inputs, unsigned char *&outputs, uint32_t &total_bytes,
                                        uint32_t testid, long &cycles, uint16_t histSize, channel_uint8_t *boundsArr,
                                        bool uniform, bool useMask, bool useAccumulation)
{
    long start, end;
    ...
    std::vector<BufferDescription> desc = {BufferDescription(Shape(histSize, 1), kIMG_NUMPY_TYPE_UINT16)};
    ...
    arm_cv_status status = arm_histogram_gray8(&input, pmaskInput, dst, histSize, &ctx, uniform, histAccum, scratch);
              ...
              uint32_t &total_bytes, long &cycles)
{
    ...
    if (status != ARM_CV_SUCCESS)
        printf("Test %ld returns an error \n", funcid);
}
